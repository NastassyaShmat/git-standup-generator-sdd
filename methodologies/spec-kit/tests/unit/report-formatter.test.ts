...
function makeGrouped(entries: Record<string, StandupEntry[]>): Map<string, StandupEntry[]> {
...
describe('formatReport()', () => {
  ...
  it('format "text" → starts with "What I did:\\n"', () => {
    const grouped = makeGrouped({ feat: [makeEntry()] });
    const { formatted } = formatReport(grouped, { ...DEFAULT_META, format: 'text' });

    assert.ok(formatted.startsWith('What I did:\n'), `Got: ${formatted.slice(0, 50)}`);
  });
  ...
  it('format "text" → contains "• <hash> <message>" entries', () => {
    ...
    assert.ok(formatted.includes('• a1b2c3d add login feature'), `Got:\n${formatted}`);
  });
  ...
  it('format "text" → ends with footer "N commit(s) across M branch(es) | period"', () => {
    ...
    assert.ok(formatted.includes('2 commit(s) across 1 branch(es) | yesterday → now'), `Got:\n${formatted}`);
  });

  it('format "text" with zero entries → "No commits found for <author> in <period>."', () => {
    const { formatted } = formatReport(new Map(), DEFAULT_META);

    assert.equal(formatted, 'No commits found for alice@example.com in yesterday → now.');
  ...
  it('format "markdown" → starts with "## What I did"', () => {
    const grouped = makeGrouped({ feat: [makeEntry()] });
    const { formatted } = formatReport(grouped, { ...DEFAULT_META, format: 'markdown' });

    assert.ok(formatted.startsWith('## What I did'), `Got: ${formatted.slice(0, 50)}`);
  });
  ...
  it('format "markdown" → entries as "- **<hash>** <message>"', () => {
    ...
    assert.ok(formatted.includes('- **a1b2c3d** add OAuth login'), `Got:\n${formatted}`);
  });

  it('format "markdown" with zero entries → "_No commits found for <author> in <period>._"', () => {
    const { formatted } = formatReport(new Map(), { ...DEFAULT_META, format: 'markdown' });

    assert.equal(formatted, '_No commits found for alice@example.com in yesterday → now._');
  });
  ...
  it('format "json" → JSON.parse(output) equals the StandupReport object', () => {
    const grouped = makeGrouped({
      feat: [makeEntry({ hash: 'a1b2c3d', message: 'add feature', type: 'feat', branch: 'main' })],
    });
    const { report, formatted } = formatReport(grouped, { ...DEFAULT_META, format: 'json' });
